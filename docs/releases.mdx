...
### UX polish (Phase B4)
- Completed B4 UX polish passes across marketing, services, legal, and case study pages.
- Standardized headers, card grids, CTA spacing, and removed remaining inline styles.

### Booking normalization (Phase B3)
- Canonical booking.doulab.net routes enforced across CTAs.

### Hardening (Passes 1-3)
- CI gating, security headers, GDPR/CH compliance notes, and custom 404/noindex.

### Reliability and legal
- Fixed broken anchors with explicit heading ids.
- Legal naming and address corrected to Santiago Arias Consulting ("Doulab"), Mettlenstrasse 45, 8193 Eglisau, Switzerland.

### Performance (Phase B5)
- Baseline capture and low-risk decoding optimizations for below-the-fold images.
- Build/verify parity captured and maintained.
...
### Booking normalization (Phase B3)
- Canonical booking.doulab.net routes (discovery/clarityscan/advisory/hdworkshop/fdworkshop + root).
- Removed Calendly/Microsoft booking links from src/** + docs/** (ops/audits excluded).

### Reliability
- Fixed broken anchors using Docusaurus Heading ids.

### UI refactor (Phase B2 Pass 1)
- Added PageHeader + CardGrid primitives.
- Reduced inline styles (Hero + FinalCta migrated to CSS modules).

### Docs parity (Phase B2 Pass 2)
- Added docs-only EntityHeader + DocBadges.
- Converted: docs/intro, docs/releases, docs/research-resources/index to MDX header pattern.
