// Viewport sweep — Phase C close-out prod-v5.2. Temporary script, not part of source tree.
// Robust harness: per-page BrowserContext teardown, retry on failure, throttle,
// resumable (skip existing non-zero PNGs), incremental structured log, CLI args.
...
if (pages.length === 0) {
  console.error('No matching pages for filter:', pageFilter);
  process.exit(2);
}
if (anchors.length === 0) {
...
try {
  for (const pg of pages) {
    ...
    for (const anchor of anchors) {
      ...
      // Resume: skip if PNG already captured non-zero.
      if (await fileExistsNonZero(file)) {
        ...
        results.push(entry);
        await flushResults();
        console.log(`${pg.slug} @ ${anchor.name} -> SKIP (exists)`);
        continue;
      }

      // Fresh context per (page, anchor) tuple.
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        // Failed attempt: record and maybe retry.
        status = attemptStatus;
      ...
      console.log(
        `${pg.slug} @ ${anchor.name} -> ${status} (${elapsed}ms) attempts=${attempts}` +
